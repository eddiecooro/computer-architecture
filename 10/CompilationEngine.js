...
class CompilationEngine {
  constructor(sourceFilePath, destWritter) {
    ...
    this.xmlWritter = new XMLWritter(destWritter);
  }
  ...
  _writeCurrentToken() {
    ...
    switch (tokenType) {
      case JackTokenizer.TOKENTYPES.IDENTIFIER:
        this.xmlWritter.writeTag('identifier', this.tokenizer.identifier());
        break;
      case JackTokenizer.TOKENTYPES.KEYWORD:
        this.xmlWritter.writeTag(
          'keyword',
          this.tokenizer.keyword().toLowerCase()
        );
        break;
      case JackTokenizer.TOKENTYPES.SYMBOL:
        this.xmlWritter.writeTag('symbol', this.tokenizer.symbol());
        break;
      case JackTokenizer.TOKENTYPES.INT_CONST:
        this.xmlWritter.writeTag('integerConstant', this.tokenizer.intVal());
        break;
      case JackTokenizer.TOKENTYPES.STRING_CONST:
        this.xmlWritter.writeTag('stringConstant', this.tokenizer.stringVal());
        break;
    }
  }
  ...
  compileClass() {
    ...
    this.xmlWritter.closeTag('class');
  }
  ...
  compileClassVarDec() {
    this.xmlWritter.openTag('classVarDec');

    this._compileKeyword(classVarDecStarters);

    this.compileRestVarDec();

    this.xmlWritter.closeTag('classVarDec');
  }
  ...
  compileSubroutineCall() {
    ...
    this._compileSymbol('(');
    this.xmlWritter.openTag('expressionList');
    if (this.tokenizer.symbol() !== ')') {
      this.compileExpressionList();
    }
    this.xmlWritter.closeTag('expressionList');
    this._compileSymbol(')');
  }
  ...
  compileReturnStatement() {
    this.xmlWritter.openTag('returnStatement');
    ...
    this.xmlWritter.closeTag('returnStatement');
  }

  compileDoStatement() {
    this.xmlWritter.openTag('doStatement');

    this._compileKeyword(JackTokenizer.KEYWORDS.DO);
    this.compileSubroutineCall();
    this._compileSymbol(';');

    this.xmlWritter.closeTag('doStatement');
  }

  compileWhileStatement() {
    this.xmlWritter.openTag('whileStatement');
    ...
    this.xmlWritter.closeTag('whileStatement');
  }

  compileIfStatement() {
    this.xmlWritter.openTag('ifStatement');
    ...
    this.xmlWritter.closeTag('ifStatement');
  }

  compileLetStatement() {
    this.xmlWritter.openTag('letStatement');
    ...
    this.xmlWritter.closeTag('letStatement');
  }

  compileStatements() {
    this.xmlWritter.openTag('statements');
    while (this.tokenizer.symbol() !== '}') {
    ...
    this.xmlWritter.closeTag('statements');
  }
  ...
  compileParameterList() {
    this.xmlWritter.openTag('parameterList');
    if (this.tokenizer.symbol() !== ')') {
    ...
    this.xmlWritter.closeTag('parameterList');
  }

  compileSubroutineBody() {
    this.xmlWritter.openTag('subroutineBody');
    this._compileSymbol('{');
    ...
    this._compileSymbol('}');
    this.xmlWritter.closeTag('subroutineBody');
  }

  compileSubroutine() {
    this.xmlWritter.openTag('subroutineDec');
    ...
    this.xmlWritter.closeTag('subroutineDec');
  }
}
